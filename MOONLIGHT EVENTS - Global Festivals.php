...
<?php
  ...
  $db = get_db();
  @$db->query("UPDATE visitor_count SET total_visits = total_visits + 1 WHERE id
= 1"); $visitor_total = 0; if ($res = @$db->query("SELECT total_visits FROM
visitor_count WHERE id = 1")) { if ($row = $res->fetch_assoc()) { $visitor_total
= (int)$row['total_visits']; } $res->free(); } ?>
<!DOCTYPE html>
<html lang="en-US" class="js scheme_light">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    <!-- Festival Categories by Religion -->
    <section class="festival-categories" style="background: var(--white)">
      <div class="container">
        <h2 class="section-title">Festivals by Religion</h2>
